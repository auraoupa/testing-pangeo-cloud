...
# Xscale
import sys
sys.path.insert(0, "/home/ajayi/xscale-fft/xscale/")
import xscale.spectral._utils 
# Warnings
import warnings
...
def fft(array, dim=None, nfft=None, dx=None, detrend=None, tapering=False,
	...
	"""
	temp_nfft, new_dim = xscale.spectral._utils.infer_n_and_dims(array, nfft, dim)
	new_nfft = xscale.spectral._utils.infer_arg(temp_nfft, dim)
	new_dx = xscale.spectral._utils.infer_arg(dx, dim)
	if detrend is 'mean':
...
def ifft(spectrum_array, dim=None, n=None, shift=True, real=True, chunks=None):
	...
	dimensions are computed with the classic fft.
	"""
	_, new_dim = xscale.spectral._utils.infer_n_and_dims(spectrum_array, n, dim)
	new_n = xscale.spectral._utils.infer_arg(n, new_dim, default_value=None)
...
def _fft(array, dim, nfft, dx, shift=False, sym=True, chunks=None):
	...
	for di in dim:
		if di in array.dims:
			...
			if dx[di] is None:
				dx[di] = xscale.spectral._utils.get_dx(array, di)
			#FFT part
